...
Since gilbert-file is a subset of the VinylFile interface it has no runtime dependencies. MIME type lookup data is generated from `mime-db` during development/build time and bundled with the package, so consumers do not need a separate `mime` dependency. It also does not suffer from extremely outdated libraries and comes with more than 30 tests.
