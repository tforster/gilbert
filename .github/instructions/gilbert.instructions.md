...
- Main Gilbert API entry point is async: `gilbert.start()` returns a `Promise` that resolves to a `ReadableStream`, so callers must `await gilbert.start()` before using stream methods
